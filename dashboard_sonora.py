...
# ============================================
def get_connection():
    """Estabelece conexão com o MySQL (XAMPP)"""
    try:
        ...
        st.info("Verifique se o XAMPP está rodando com o MySQL ativo.")
        return None
...
st.sidebar.info(
    ...
    **Base:** MySQL + Streamlit + CSV Público
    """
)
    ...
    with tab3:
        ...
        if len(musicas_edicao) > 0:
            ...
            musica_selecionada = pd.read_sql(
                "SELECT name, singer, url FROM youtube_musics WHERE id = %s",
                conn,
    ...
    with tab4:
        ...
        if len(musicas_lista) > 0:
            ...
            if st.button("Remover", type="secondary"):
                cursor = conn.cursor()
                cursor.execute("DELETE FROM youtube_musics WHERE id = %s", (musica_remover,))
                conn.commit()
    ...
    with tab3:
        ...
        if len(eventos_edicao) > 0:
            ...
            evento_selecionado = pd.read_sql(
                "SELECT event_name, location, start_date FROM events WHERE id = %s",
                conn,
            ...
            with st.form("editar_evento", clear_on_submit=True):
                ...
                if submitted:
                    if novo_nome_evento:
                        ...
                        cursor.execute("""
                            UPDATE events
                            SET event_name = %s, location = %s, start_date = %s, end_date = %s
                            WHERE id = %s
    ...
    with tab4:
        ...
        if len(eventos_lista) > 0:
            ...
            if st.button("Remover Evento", type="secondary"):
                cursor = conn.cursor()
                cursor.execute("DELETE FROM events WHERE id = %s", (evento_remover,))
                conn.commit()
        ...
        with st.form("novo_pedido", clear_on_submit=True):
            ...
            if submitted:
                ...
                ordem_existe = pd.read_sql(
                    "SELECT COUNT(*) FROM music_order WHERE event_id = %s AND `order` = %s",
                    conn,
                    params=(evento, ordem)
                ).iloc[0, 0]
                ...
                if ordem_existe > 0:
                    ...
                    try:
                        ...
                        cursor.execute("""
                            ...
                            VALUES (UUID(), %s, %s, %s, 'accepted', %s, TRUE)
